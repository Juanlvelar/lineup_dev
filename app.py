...
st.markdown("Genera rotaciones equilibradas asegurando que cada jugador juegue fuera del arco el mismo tiempo, con PDF profesional y posibilidad de editar cada intervalo.")
...
num_players = st.sidebar.slider("Number of players", 6, 10, 8)
...
if st.button("🎲 Generate Rotations"):
    if len(players) < 6:
        ...
        for attempt in range(800):
            ...
            diff = max(minutes_played.values()) - min(minutes_played.values())
            if diff < best_diff:
                best_diff = diff
...
# --- MOSTRAR INTERVALOS (SI EXISTEN) ---
if "edited_lineups" in st.session_state:
    edited_lineups = st.session_state["edited_lineups"]
    all_players = list(players.keys())
    field_positions = list(formation_x.keys())

    st.header("🕐 Match Intervals")
    for i, lineup in enumerate(edited_lineups):
        st.subheader(f"Interval {i+1}")
        cols = st.columns(len(field_positions))
        for j, pos in enumerate(field_positions):
            with cols[j]:
                selected = st.selectbox(f"{pos}", all_players, index=all_players.index(lineup[pos]) if lineup[pos] in all_players else 0, key=f"edit_{i}_{pos}")
                lineup[pos] = selected

        resting_players = [p for p in all_players if p not in lineup.values()]
        st.write("Resting:", ", ".join(resting_players) if resting_players else "None")
    ...
    # --- REGENERAR MINUTOS TRAS EDICIÓN ---
    minutes_played = defaultdict(int)
    gk_count = defaultdict(int)
    for lineup in edited_lineups:
